...
- (void) setPickerType:(SBPickerSelectorType)pickerType{
    ...
    }else if (pickerType == SBPickerSelectorTypeCountry) {
        [self.pickerView removeFromSuperview];
        [self.datePickerView removeFromSuperview];
        [self.view addSubview:self.countryPickerView];
        
        frame = self.countryPickerView.frame;
        frame.origin.y = CGRectGetMaxY(self.optionsToolBar.frame);
        self.countryPickerView.frame = frame;
    }else{
        [self.countryPickerView removeFromSuperview];
        [self.datePickerView removeFromSuperview];
...
#pragma GCC diagnostic pop
    ...
    if (self.pickerType == SBPickerSelectorTypeCountry) {
        if (self.delegate) {
            NSMutableString *str = [NSMutableString stringWithString:@""];
            for (int i = 0; i < self.numberOfComponents; i++) {
                if (self.numberOfComponents == 1) {
                    [str appendString:[CountryPicker countryNames][[self.countryPickerView selectedRowInComponent:0]]];
                }else{
                    NSMutableArray *componentData = [CountryPicker countryNames][i];
                    [str appendString:componentData[[self.countryPickerView selectedRowInComponent:i]]];
                    if (i<self.numberOfComponents-1) {
                        [str appendString:@" "];
                    }
                }
            }
            if ([self.delegate respondsToSelector:@selector(pickerSelector:selectedCountryName:countryCode:index:)]) {
                [self.delegate pickerSelector:self selectedCountryName:str countryCode:@"" index:[self.countryPickerView selectedRowInComponent:0]];
            }
            [self dismissPicker];
            return;
        }
...
{
    if ([self.delegate respondsToSelector:@selector(pickerSelector:intermediatelyCoutryName:countryCode:index:)]) {
        [self.delegate pickerSelector:self intermediatelyCoutryName:name countryCode:code index:[self.countryPickerView selectedRowInComponent:0]];
    }
}
...
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
        if (self.delegate && [self.delegate respondsToSelector:@selector(SBPickerSelector:intermediatelySelectedValue:atIndex:)]) {
            [self.delegate SBPickerSelector:self intermediatelySelectedValue:sender.date atIndex:0];
        }
#pragma GCC diagnostic pop
        
        if (self.delegate && [self.delegate respondsToSelector:@selector(pickerSelector:intermediatelySelectedValue:atIndex:)]) {
            [self.delegate pickerSelector:self intermediatelyDateValue:sender.date atIndex:0];
        }
